...
class master_swarm:
    ...
    def __init__(self,num_cf, if_heading, if_desired):
        ...
        self.map = sio.loadmat('/home/tugay/Environments/circle_4x65.mat')
        self.map = self.map['I']
        self.size_x = 6.5
        self.size_y = 4

        # self.map = sio.loadmat('/home/tugay/Environments/circle_8x23.mat')
        # self.map = self.map['I']
        # self.size_x = 23
        # self.size_y = 8
        ...
        v2 = np.array([self.size_x,0.0])
        self.vertices.append(v2)
        v3 = np.array([self.size_x,self.size_y])
        self.vertices.append(v3)
        v4 = np.array([0.0,self.size_y])
    ...
    def calc_heading(self):
        sum_cosh = np.zeros((1,num_cf))
        sum_sinh = np.zeros((1,num_cf))

        for i in range(0,num_cf):
            sum_cosh[0,i] = np.cos(self.h[0,i])
            sum_sinh[0,i] = np.sin(self.h[0,i])

            for j in range(0,num_cf):
                if self.d_ij[i,j] < self.Da and i != j:

                    sum_cosh[0,i] = sum_cosh[0,i] + np.cos(self.h[0,j])
                    sum_sinh[0,i] = sum_sinh[0,i] + np.sin(self.h[0,j])

        self.hbar_x = np.divide(sum_cosh , np.sqrt(np.square(sum_cosh) + np.square(sum_sinh)))  
        self.hbar_y = np.divide(sum_sinh , np.sqrt(np.square(sum_cosh) + np.square(sum_sinh))) 
    ...
    def calc_grad(self):
        ...
        # self.grad_x[self.grad_x >= 200] = 199
        # self.grad_x[self.grad_x <= 0] = 0
        #
        # self.grad_y[self.grad_y >= 575] = 574
        # self.grad_y[self.grad_y <= 0] = 0
    ...
    def calc_force(self):
        ...
        self.f_x = self.alpha*self.pbar_xs + self.beta*self.if_heading*self.hbar_x + self.gama*self.r_x
        self.f_y = self.alpha*self.pbar_ys+ self.beta*self.if_heading*self.hbar_y + self.gama*self.r_y 
    ...
    def publish_velocities(self):
        ...
        for j in range(0,num_cf):
            ...
            cfs[j].cmdHover(vx=self.vel_cmd[0],vy=self.vel_cmd[1],yawRate=0,zDistance=0.5)
